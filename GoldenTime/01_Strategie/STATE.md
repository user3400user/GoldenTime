...
**Stand: 18.06.2026 · Phase: Vollpaket gebaut + demo-fertig; unabh. Standortbestimmung + konvergierter Umsetzungs-Plan (M0→M3) liegt vor (Claude Code, `main`) · Nächster Meilenstein: Berater-Essen ~27.06. (= M0)**
...
**Unabh. Standortbestimmung + Umsetzungs-Plan (17.–18.06., Multi-Agent-Workflows, read-only):** Adversariale Gap-Analyse (9 Dim., skeptiker-verifiziert) → **Schnitt Ist ≈ 2,6/5**. Am Code verifizierte Kern-Befunde: **USP (Exklusivität/Dedupe) im Code NICHT erzwungen** (`ledger.record_delivery` 0 Produktions-Aufrufer; `deliver.run_region` berührt Ledger nicht; `cmd_signals` cli.py:188-192 = 4. ungeschützter Funnel u. einziger heutiger `filter_deliverable`-Aufrufer); **T1/T4-Diff lief nie** (nur 1 Snapshot); atomarer Snapshot-Write + Diff-Klassifikation korrekt; **14 mypy-Fehler = False Positives**. **Recht-Korrektur:** dl-de-Resale **erlaubt** (govdata.de, nur Quellenvermerk) → echter Launch-Blocker ist **DSGVO** (Art-6(1)(f)-Abwägung für e.K.; EuGH C-621/22, OLG Stuttgart 2 U 63/22). Daraus via **3-Varianten-Plan-Turnier + Synthese + Verifikation** ein konvergierter Plan (3 Dateien, außerhalb Repo unter `~/`): **Zielbild** (Endbild je Dim. 4,5–5,0) · **Sprint-Plan** (S0–S3 + PT1, prüfbare Exits) · **Roadmap-Fern** (M2/M3 + Provenienz). **Meilensteine M0(Demo, ~erreicht) → M1(1. zahlender Kunde) → M2 → M3**; kritischer Pfad zu M1 = **Kalender (Anwalt), nicht Code**; **T1/T4-Beweis & volles CI-Gate sind NICHT M1** (→ M2). **G5-Fix asymmetrisch** (status==APPROVED + neuer QA-Flag → PENDING via `flags_at_review`), **NICHT** Flags in `fingerprint()` (bräche R6-Design qa_gate.py:155). e.K. = **hartes, qa-unabhängiges Liefer-Gate** in `run_region`+`cmd_signals`, config-geflaggt.
**Offen (= Plan S0–S3/PT1):** **record_delivery/reserve verdrahten = S1/S2** (USP erzwingen, `--commit`/`--dry-run`, Transaktion); **DSGVO-Anwalt = M1-Vorbedingung (PT1), bei erster Zusage starten**; e.K. hart filtern + lieferbare Menge nach Filter **messen** (Go/No-Go); FAKTURIEREN/FLUSS-Zahlen → M2; Mensch-QA vor dem Essen (`qa suggest`, ~70–90 Grenzfälle). Tech-Gedächtnis: `CLAUDE.md`. Plan-Dateien: `~/GoldenTime-{Zielbild,Sprint-Plan,Roadmap-Fern}.md`.
