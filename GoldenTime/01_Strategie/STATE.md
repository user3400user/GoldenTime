...
**Build — PULL/B-Backbone (CC, 14.06.):** `02_Daten/pipeline/` steht — Export-Adapter (open-mastr -> SQLite) + ABR-Speicher-Anywhere-Query (3-Wege: „kein Speicher gemeldet" / Premium-Speicher anderswo / colocated = Ausschluss) + Normalisierung (Trigger T1/T2/T3, `Inbetriebnahmedatum`-Frische-Validierung) + CLI (`inspect`/`build-db`/`leads`) + Tests 9/9 gruen (synthetisches SQLite, ohne 3-GB-Download/open-mastr). Speicher-Check jetzt ID-basiert betreiberweit statt PLZ-Fuzzy. Technik-Gedaechtnis: `CLAUDE.md`. Offen: echter `build-db`-Lauf + `inspect`-Schema-Verifikation auf dem ZBook, dann QUALIFIZIEREN (Lead-Spec §2).
